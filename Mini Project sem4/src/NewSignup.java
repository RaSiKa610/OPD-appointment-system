...
class NewSignup extends JFrame {
    NewSignup() {
        ...
        bloodGroupBox.setBounds(fieldX, yStart + 6 * gap, width, height);
                ...
                a -> {
                    if (t2.getText().equals(t3.getText())) {
                        ...
                        try (Connection con = DriverManager.getConnection(url, "root", "Shlok2401@")) {
                            String sql = "INSERT INTO patients(username, password, phone, email, age, blood_group, gender) VALUES(?, ?, ?, ?, ?, ?, ?)";
                            try (PreparedStatement pst = con.prepareStatement(sql)) {
                                ...
                                pst.setInt(5, Integer.parseInt(t6.getText())); // Age
                                pst.setString(6, (String) bloodGroupBox.getSelectedItem()); // Blood Group
                                pst.setString(7, (String) genderBox.getSelectedItem()); // Gender

                                pst.executeUpdate();
                                JOptionPane.showMessageDialog(null, "Signup Successful");
                                new MAppointment(t1.getText());
                                dispose();
                            }
                        } catch (Exception e) {
